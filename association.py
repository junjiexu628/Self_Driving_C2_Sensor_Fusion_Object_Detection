...
class Association:
    ...
    def get_closest_track_and_meas(self):
        ...
        #find the minimum value for index i & j
        min_ij = np.unravel_index(np.argmin(A, axis =None), A.shape)
        id_i = min_ij[0]
        id_j = min_ij[1]
        
        # delete the row and colum on index i & j
        A = np.delete(A, id_i, 0)
        A = np.delete(A, id_j, 1)
        self.association_matrix = A
        
        update_track = self.unassigned_tracks[id_i]
        update_meas = self.unassigned_meas[id_j]
